...
describe('pathUnitTest suite', function() {
	...
	it('Test Vertices', function(done) {
		assert.deepEqual(path.vertices[0],new g.Vertex('n', 7.3,{}));
		assert.deepEqual(path.vertices[1],new g.Vertex('n', 7.9,{}));
		done();
	});

	it('Test Edges', function(done) {
		assert.deepEqual(path.edges[0],new g.Edge('r', 5.7, 7.3, 7.9, {}));
		done();
	});
